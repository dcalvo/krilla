...
impl Repr {
    fn bits_per_component(&self) -> u8 {
        match self {
            ...
            Repr::Precompressed(p) => p.bits_per_component.as_u8(),
        }
    }
}
...
impl Image {
    ...
    pub(crate) fn serialize(
        ...
        let chunk = Deferred::new(move || {
            ...
            // For Sampled/Jpeg/Png we go through FilterStreamBuilder so the encoded data
            // and filter list are paired. Precompressed bypasses it: bytes are already
            // encoded by the source PDF and the filter names are written verbatim.
            enum ImageStream<'a> {
            ...
            let image_stream = match repr {
                ...
                Repr::Png(p) => ImageStream::Built(
                    FilterStreamBuilder::new_from_png_data(
                        p.data.as_ref(),
            ...
            let precompressed_indexed = match repr {
            ...
            } else if let Some(idx) = precompressed_indexed {
